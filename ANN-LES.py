...
model1 = tf.keras.models.load_model('1wend1.keras')
model2 = tf.keras.models.load_model('1wend3.keras')
model3 = tf.keras.models.load_model('1wend3.keras')
model4 = tf.keras.models.load_model('1wend4.keras')
model5=tf.keras.models.load_model('1wend4.keras')
@njit
def feq(k, rho, u):
...
def getT(u,A,inPut):
    for i in range(128):
        for j in range(128):
            inPut[i][j] = np.array([[
                np.sqrt(u[i][j][0] * u[i][j][0] + u[i][j][1] * u[i][j][1]), A[i][j][0] + A[i][j][3],
                np.sqrt(
...
def main():
    ...
    while True:
        ...
        if n % 1000 == 0 and n>=2000:
            ...
            #T1 = T1.reshape(128*128, 8)
            T_log = T1
            pren3=model5.predict(T_log)
            pre1 = model1.predict(T_log)
            pre2 = model2.predict(T_log)
            pre3 = model3.predict(T_log)
            pre4 = model4.predict(T_log)
            pre1=-np.exp(pre1)
            pre4=-np.exp(pre4)
            pre2 = np.exp(pre2)
            pre3 = np.exp(pre3)
